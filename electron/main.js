...
function getLogFilePath() {
  ...
  if (isDev) {
    ...
    return path.join(__dirname, '..', 'logs', 'zhike_phoneagent_{time:YYYY-MM-DD}.log');
  } else {
    // 打包模式：使用用户数据目录（跨平台标准位置）
    // Windows: %APPDATA%/ZHIKE-PhoneAgent/logs/
    // macOS: ~/Library/Application Support/ZHIKE-PhoneAgent/logs/
    // Linux: ~/.config/ZHIKE-PhoneAgent/logs/
    return path.join(app.getPath('userData'), 'logs', 'zhike_phoneagent_{time:YYYY-MM-DD}.log');
  }
}
...
async function startBackend() {
  ...
  if (isDev) {
    ...
    if (process.platform === 'win32') {
      backendExe = path.join(backendDir, 'zhike-phoneagent.exe');
    } else {
      backendExe = path.join(backendDir, 'zhike-phoneagent');
    }
  ...
  const env = {
    ...
    // For PyInstaller-frozen backend, UTF-8 mode is set via build-time OPTIONS in zhike-phoneagent.spec
...
function createSplash() {
  ...
  writeMainLog('info', '[Window] Splash window created');
}
...
function closeSplash() {
  if (splashWindow && !splashWindow.isDestroyed()) {
    splashWindow.close();
    splashWindow = null;
    writeMainLog('info', '[Window] Splash window closed');
  }
}
...
function createWindow() {
  ...
  webContents.on('console-message', (event, level, message, line, sourceId) => {
    ...
    } else if (process.env.ZHIKE_DEBUG_RENDERER === '1') {
  ...
  const enableDevTools = process.argv.includes('--dev') || process.env.ZHIKE_PERF === '1';
  if (enableDevTools) {
...
app.whenReady().then(async () => {
  try {
    ...
    console.log('ZHIKE-PhoneAgent 正在启动...');
