...
import zhike_phoneagent.layered_agent_service as layered_service
from zhike_phoneagent.actions import ActionHandler
from zhike_phoneagent.devices.adb_device import ADBDevice
from zhike_phoneagent.metrics import get_metrics_registry, reset_trace_latency_metrics
...
def test_layered_task_trace_observability_covers_debug_surface(
    ...
    import zhike_phoneagent.adb.device as adb_device_module
    ...
    try:
        ...
        assert {
            "zhike.task.start",
            "zhike.layered.tool_call",
            "zhike.layered.tool_result",
            "zhike.layered.message",
            "zhike.task.done",
            "zhike.trace.summary",
            "zhike.task.status",
        } <= replay_event_names
        tool_call_record = next(
            ...
            if record["event_name"] == "zhike.layered.tool_call"
        )
