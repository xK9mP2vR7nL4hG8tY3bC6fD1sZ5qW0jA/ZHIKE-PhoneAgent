#!/usr/bin/env python3
"""
ZHIKE-PhoneAgent Electron 一键构建脚本
...
class ElectronBuilder:
    ...
    def build_frontend(self) -> bool:
        ...
        backend_static = self.root_dir / "zhike_phoneagent" / "static"
    ...
    def build_backend(self) -> bool:
        ...
        # 清理旧的构建输出
        pyinstaller_dist = self.scripts_dir / "dist" / "zhike-phoneagent"
        pyinstaller_build = self.scripts_dir / "build" / "zhike-phoneagent"
        ...
        print("\n运行 PyInstaller...")
        if not run_command(
            ["uv", "run", "pyinstaller", "zhike-phoneagent.spec"], cwd=self.scripts_dir
        ):
            return False

        # 复制到 resources/backend
        print("\n复制后端到 resources...")
        backend_dist = self.scripts_dir / "dist" / "zhike-phoneagent"
    ...
    def build_electron(self) -> bool:
        ...
        # 构建 Electron
        # 始终使用 --publish never：electron-builder 内置 publish 在 Windows 上
        # 上传大文件时容易超时（GitHub Actions runner 网络不稳定）。产物上传
        # 改为在 Release 工作流中通过 `gh release upload` 单独处理。
        build_cmd = ["pnpm", "run", "build", "--publish", "never"]
    ...
    def build(self) -> bool:
        """执行完整构建流程"""
        print(f"\n{Color.BOLD}ZHIKE-PhoneAgent Electron 构建工具{Color.RESET}")
