...
"""
PyInstaller 配置文件 - ZHIKE-PhoneAgent 后端打包

使用方法:
    cd scripts
    pyinstaller zhike-phoneagent.spec

输出目录:
    scripts/dist/zhike-phoneagent/
"""
...
a = Analysis(
    ...
    [str(ROOT_DIR / 'zhike_phoneagent' / '__main__.py')],
    ...
    binaries=[
        ...
        (str(ROOT_DIR / 'zhike_phoneagent' / 'resources' / 'scrcpy-server-v3.3.3'), 'zhike_phoneagent/resources'),
    ],

    # 数据文件
    datas=[
        # 前端静态文件（必需）
        (str(ROOT_DIR / 'zhike_phoneagent' / 'static'), 'zhike_phoneagent/static'),

        # ADB Keyboard APK 及许可证文件（自动安装功能）
        (str(ROOT_DIR / 'zhike_phoneagent' / 'resources' / 'apks'), 'zhike_phoneagent/resources/apks'),
