...
from zhike_phoneagent.adb_plus.qr_pair import qr_pairing_manager
...
def _get_static_dir() -> Path | None:
    ...
    if meipass:
        bundled_static = Path(meipass) / "zhike_phoneagent" / "static"
    ...
    try:
        from zhike_phoneagent import __file__ as package_file
    ...
    try:
        static_dir = files("zhike_phoneagent").joinpath("static")
...
def create_app() -> FastAPI:
    ...
    log_level = os.getenv("ZHIKE_LOG_LEVEL", "INFO")
    log_file = (
        None
        if os.getenv("ZHIKE_NO_LOG_FILE")
        else os.getenv("ZHIKE_LOG_FILE", "logs/zhike_phoneagent_{time:YYYY-MM-DD}.log")
    )
    ...
    async def combined_lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
        ...
        adb_path = os.environ.get("ZHIKE_ADB_PATH", "adb")
    ...
    app = FastAPI(
        title="ZHIKE-PhoneAgent API", version=APP_VERSION, lifespan=combined_lifespan
    )
