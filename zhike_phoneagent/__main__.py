"""CLI entry point for ZHIKE-PhoneAgent."""
...
from zhike_phoneagent import __version__
from zhike_phoneagent.adb_terminal_repl import main as adb_terminal_repl_main
...
def main() -> None:
    """Start the ZHIKE-PhoneAgent server."""
    ...
    early_parser.add_argument(
        "--log-file", default="logs/zhike_phoneagent_{time:YYYY-MM-DD}.log"
    )
    ...
    os.environ["ZHIKE_LOG_LEVEL"] = early_args.log_level
    if early_args.no_log_file:
        os.environ["ZHIKE_NO_LOG_FILE"] = "1"
    else:
        os.environ["ZHIKE_LOG_FILE"] = early_args.log_file

    # Import and configure logger FIRST
    from zhike_phoneagent.logger import configure_logger
    ...
    parser = argparse.ArgumentParser(
        description="ZHIKE-PhoneAgent - AI-powered Android automation and phone agent platform"
    )
    parser.add_argument(
        "--base-url",
        required=False,
    ...
    parser.add_argument(
        "--apikey",
        default=None,
        help="API key for the model API (default: from ZHIKE_API_KEY or unset)",
    )
    parser.add_argument(
        "--host",
        default="127.0.0.1",
    ...
    parser.add_argument(
        "--log-file",
        default="logs/zhike_phoneagent_{time:YYYY-MM-DD}.log",
        help="Log file path (default: logs/zhike_phoneagent_{time:YYYY-MM-DD}.log)",
    )
    parser.add_argument(
        "--no-log-file",
        action="store_true",
    ...
    from zhike_phoneagent import server
    ...
    print()
    print("=" * 50)
    print("  ZHIKE-PhoneAgent - Phone Agent Web Interface")
    print("=" * 50)
    ...
    # 确保 ADB 可用
    from zhike_phoneagent.adb_manager import ensure_adb

    try:
        adb_path = ensure_adb()
    except RuntimeError as e:
        print(f"\n[ZHIKE] WARNING: {e}", file=sys.stderr)
        adb_path = "adb"  # 降级，让后续错误正常暴露

    # 写入环境变量（供 --reload 模式的子进程使用）
    os.environ["ZHIKE_ADB_PATH"] = adb_path
    os.environ["ZHIKE_SERVER_HOST"] = args.host
    ...
    uvicorn.run(
        server.app if not args.reload else "zhike_phoneagent.server:app",
        host=args.host,
        port=args.port,
        reload=args.reload,
